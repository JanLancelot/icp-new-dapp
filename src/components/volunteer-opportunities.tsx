    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {opportunities.map((opportunity, index) => (
          <div
            key={index}
            className={`relative overflow-hidden rounded-lg transition-transform duration-300 ease-in-out transform hover:scale-105 ${
          ...
          >
            <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center text-white p-6 transition-all duration-300 ease-in-out hover:bg-opacity-70">
              <div className="transition-transform duration-300 ease-in-out transform hover:scale-110 hover:rotate-6">
                <opportunity.icon className="w-12 h-12 mb-4" />
              </div>
              <h3 className="text-xl font-semibold text-center transition-all duration-300 ease-in-out hover:text-2xl">{opportunity.title}</h3>
            </div>
          </div>
        ))}
      </div>
